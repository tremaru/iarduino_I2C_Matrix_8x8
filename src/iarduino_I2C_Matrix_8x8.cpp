...
//			Определяем значения переменных:																				//	
			...
			data[0]=0;   _writeBytes(REG_SYMBOL_INPUT,1);																//	Загружаем 0 в регистр «SYMBOL_INPUT», тогда символ с кодом 0 отобразится на дисплее, но это не символ, а значения ширины, интервала и отступа символов.
...
//				Записываем номер функции анимации:																		//
				_writeBytes(REG_FUNCTIONS,1);																			//	Записываем 1 байт в регистр «FUNCTIONS» из массива data.
//				Ждём сброса регистра анимации:																			//
				do{ _readBytes(REG_FUNCTIONS,1); delay(500); }while( data[0] > 0 );										//	Читаем 1 байт регистра «FUNCTIONS» в массив data, пока значение этого регистра не будет сброшено.
//			Очистка дисплея без анимации:																				//
			}else{																										//
//				Останавливаем бегущую строку:																			//
				data[0]=0; _writeBytes(REG_TIME_STEP,1);																//	Сбрасываем регистр «TIME_STEP» для остановки бегущей строки, если она сейчас выводится.
//				Устанавливаем бит очистки дисплея:																		//
				_readBytes(REG_DATA,1);																					//	Читаем 1 байт регистра «REG_DATA» в массив data.
				data[0] |= 0b00010000;																					//	Устанавливаем бит «CLEAR_SCR»
				_writeBytes(REG_DATA,1);																				//	Записываем 1 байт в регистр «REG_DATA» из массива data.
//				Ждём сброса бита очистки дисплея:																		//
				do{ _readBytes(REG_DATA,1); }while( (data[0]&0b00010000) > 0 );											//	Читаем 1 байт регистра «REG_DATA» в массив data, пока в нём не сбросится бит «CLEAR_SCR».
			}																											//
		}																												//
}																														//
																														//
...
//				Устанавливаем бит очистки дисплея:																		//
				...
				_writeBytes(REG_DATA,1);																				//	Записываем 1 байт в регистр «REG_DATA» из массива data.
//				Ждём сброса бита очистки дисплея:																		//
				do{ _readBytes(REG_DATA,1); }while( (data[0]&0b00100000) > 0 );											//	Читаем 1 байт регистра «REG_DATA» в массив data, пока в нём не сбросится бит «FILL_SCR».
			}																											//
		}																												//
}																														//
																														//
...
void	iarduino_I2C_Matrix_8x8::invScr			(void){																	//	Параметр:				отсутствует.
		if(valAddr){																									//	Если дисплей был инициализирован, то ...
			...
			_readBytes(REG_DATA+1,8);																					//	Читаем 8 байт начиная с регистра «REG_DATA_1» в массив data.
			for(int i=0; i<8; i++){ data[i] = ~data[i]; }																//	Инвертируем все биты в 8 элементах массива data.
			_writeBytes(REG_DATA+1,8);																					//	Записываем 8 байт в регистры начиная с «REG_DATA_1» из массива data.
		}																												//
}																														//
																														//
...
//					Если указана функция появления из пустого фона:														//	
					data[0]=1; _writeBytes(REG_FUNCTIONS,1);															//	Загружаем номер анимации предварительной очистки дисплея в регистр «FUNCTIONS».
				}else{																									//	X8_FILLED_...: 102, 106, 110 ...
//					Если указана функция появления из закрашенного фона:												//	
					data[0]=2; _writeBytes(REG_FUNCTIONS,1);															//	Загружаем номер анимации предварительной заливки дисплея в регистр «FUNCTIONS».
				}																										//
...
//			Загружаем изображение:																						//
			...
			_writeBytes(REG_DATA+1,8);																					//	Записываем 8 байт массива data в модуль начиная с регистра «REG_DATA_1».
//			Если указано использовать анимацию появления, то:															//
			if(func){																									//
//				Определяем и загружаем номер функции в регистр анимации:												//
				data[0]=func-100+3;																						//	100=>3, 102=>5, 104=>7, 106=>9, 108=>B, 110=>D ...
				_writeBytes(REG_FUNCTIONS,1);																			//
//				Ждём сброса регистра анимации:																			//
				do{ _readBytes(REG_FUNCTIONS,1); delay(500); }while( data[0] > 0 );										//	Читаем 1 байт регистра «FUNCTIONS» в массив data, пока значение этого регистра не будет сброшено.
			}																											//
		}																												//
...
//			Загружаем цифру:																							//
			data[0]=num+0x30; _writeBytes(REG_SYMBOL_INPUT,1);															//	Загружаем код символа цифры в регистр «SYMBOL_INPUT», тогда символ цифры отобразится на дисплее.
//			Если указано использовать анимацию появления, то:															//
			if(func){																									//
//				Определяем и загружаем номер функции в регистр анимации:												//
				data[0]=func-100+3;																						//	100=>3, 102=>5, 104=>7, 106=>9, 108=>B, 110=>D ...
...
//			Если str содержит 1 символ, то выводим его на дисплей:														//
			if((str.length()==1)||(str.length()==2&&codingName==X8_TXT_UTF8&&(uint8_t(str[0])==208||uint8_t(str[0])==209))){// Если в str 1 символ или (в str 2 символа в кодировке UTF8 и код первого символа равен 208 или 209), то ...
				data[0]=_codingCP866(str).charAt(0); _writeBytes(REG_SYMBOL_INPUT,1);									//	Загружаем код символа в регистр «SYMBOL_INPUT», тогда символ отобразится на дисплее.
...
//				Ждём сброса бита очистки текста:																		//
				do{ _readBytes(REG_DATA,1); }while( (data[0]&0b00001000) > 0 );											//	Читаем 1 байт регистра «REG_DATA» в массив data, пока в нём не сбросится бит «CLEAR_STR».
...
//				Загружаем новую строку посимвольно:																		//
				for(int i=0; i<stringLen; i++){																			//	Проходим по всем символам строки str в кодировке CP866 ...
					data[0]=_codingCP866(str).charAt(i); _writeBytes(REG_TEXT_INPUT,1);									//	Извлекаем код очередного символа из строки str и загружаем его в регистр «TEXT_INPUT».
				    do{ _readBytes(REG_TEXT_INPUT,1); }while( data[0] > 0 );											//	Читаем 1 байт регистра «TEXT_INPUT» в массив data, пока значение этого регистра не будет сброшено.
				}	data[0]=0; _writeBytes(REG_TEXT_INPUT,1);															//	Загружаем код символа конца строки в регистр «TEXT_INPUT».
			}																											//
//			Если указано использовать анимацию появления, то:															//
			if(func){																									//
//				Определяем и загружаем номер функции в регистр анимации:												//
...
void	iarduino_I2C_Matrix_8x8::autoScroll		(uint8_t speed, uint16_t pause){										//	Параметр:				скорость от 0 до 255, [пауза от 0 до 25500 мс]).
		if(valAddr){																									//	Если дисплей был инициализирован, то ...
			...
			_writeBytes(REG_TIME_STEP,2);																				//	Загружаем 2 байта массива data в модуль начиная с регистра «TIME_STEP».
		}																												//
}																														//
																														//
...
//			Записываем значение в регистр сдвига бегущей строки:														//
			...
			_writeBytes(REG_STEP_LEN,2);																				//	Записываем 2 байта в двухбайтный регистр «STEP_LEN» из массива data.
//			Ждём сброса регистра сдвига бегущей строки:																	//
			do{ _readBytes(REG_STEP_LEN,2); }while( (data[0]>0)||(data[1]>0) );											//	Читаем 2 байта двухбайтного регистра «REG_DATA» в массив data, пока оба байта не сбросятся в 0.
		}																												//
}																														//
																														//
...
void	iarduino_I2C_Matrix_8x8::scrollStep		(void){																	//	Параметр:				отсутствует.
		if(valAddr){																									//	Если дисплей был инициализирован, то ...
			...
			_writeBytes(REG_STEP_LEN+1,1);																				//	Записываем 1 байт в старший байт двухбайтного регистра «STEP_LEN» из массива data.
		}																												//
}																														//
																														//
...
void	iarduino_I2C_Matrix_8x8::setTimeIdleFirst(uint16_t time){														//	Параметр:				время простоя в мс.
		if(valAddr){																									//	Если дисплей был инициализирован, то ...
			...
			_writeBytes(REG_TIME_START,1);																				//	Загружаем 1 байт массива data в регистр «TIME_START».
		}																												//
}																														//
																														//
...
void	iarduino_I2C_Matrix_8x8::setTimeIdleLast(uint16_t time){														//	Параметр:				время простоя в мс.
		if(valAddr){																									//	Если дисплей был инициализирован, то ...
			...
			_writeBytes(REG_TIME_STOP,1);																				//	Загружаем 1 байт массива data в регистр «TIME_STOP».
		}																												//
}																														//
																														//
...
//			Записываем код символа в регистр перезаписи изображения символа:											//
			data[0]=i; _writeBytes(REG_SAVE_AS,1);																		//	Записываем 1 байт в регистр «SAVE_AS» из массива data.
//			Ждём сброса регистра перезаписи изображения символа:														//
			do{ _readBytes(REG_SAVE_AS,1); }while( data[0]>0 );															//	Читаем 1 байт регистра «SAVE_AS» в массив data, пока он не сбросятся в 0.
		}																												//
}																														//
																														//
...
//			Останавливаем бегущую строку:																				//
			_readBytes(REG_TIME_STEP,1); tmpTimeStep=data[0];															//	Читаем 1 байт из регистра «TIME_STEP» через массив data в переменную tmpTimeStep.
			if(tmpTimeStep>0){ data[0]=0; _writeBytes(REG_TIME_STEP,1); }												//	Если включена автопрокрутка бегущей строки (tmpTimeStep>0) то останавливаем бегущую строку сбросив регистр «TIME_STEP» в 0.
...
//			Сохраняем изображение символа с кодом 0:																	//
			...
			data[0]=0; _writeBytes(REG_SAVE_AS,1);																		//	Записываем 1 байт в регистр «SAVE_AS» из массива data.
...
//			Восстанавливаем изображение экрана на то, что было до вызова функции _setChar0():							//
			if(tmpTimeStep>0){ data[0]=tmpTimeStep; _writeBytes(REG_TIME_STEP,1); }										//	Если прокручивалась бегущая строка, то возобновляем её прокрутку.
